...
function check_os() {
    ...
    if [ X"$CPU_BIT" == X"x86_64" ]; then
        if [ X"$(cat /proc/cpuinfo | grep rdtscp | uniq)" == X"" ]; then
            echo "The cpu instruction rdtscp is missing."
        fi
    fi
}
...
function single_install() {
    ...
    gs_initdb -w $password -D $app/data/single_node --nodename "sgnode"
...
function init_db() {
    ...
    gs_initdb -D $app/data/master --nodename=datanode1 -E UTF-8 -U $user  -w $password
    info "[init slave datanode.]"
    gs_initdb -D $app/data/slave --nodename=datanode2 -E UTF-8 -U $user  -w $password
}
